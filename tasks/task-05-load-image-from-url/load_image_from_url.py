...
def load_image_from_url(url: str, flags: int = cv.IMREAD_COLOR) -> np.ndarray:
    ...
    try:
        ### START CODE HERE ###
        ### TODO
        raw_image = urllib.request.urlopen(url)
        array1d = np.asarray(bytearray(raw_image.read(), dtype=np.uint8))
        image = cv.imdecode(array1d, flags)
        if image is None:
            raise RuntimeError("Erro em decodificar")
        ### END CODE HERE ###
