...
def get_image_info(image):
    ...
    if depth == 1:
        # Single-channel
        info["statistics"]["min"] = int(image.min())
        info["statistics"]["max"] = int(image.max())
        info["statistics"]["mean"] = float(image.mean())
        info["statistics"]["std_dev"] = float(image.std())
